    ...
    if request.method == 'POST':
        ...
        try:
            new_report = DrugReport(
                ...
                listOfEnvolvedPeople='[Marcelo, Maria]',#request.POST.getlist('listOfEnvolvedPeople'),
                listItensLabels='[Item1, Item2]',#request.POST.getlist('listItensLabels'),
                listOfPackagings='[tubo, plástico]',#request.POST.getlist('listOfPackagings'),
                listOfMorphology='[pó, erva]',#request.POST.getlist('listOfMorphology'),
                listOfGrossMass='[30, 20]',#[float(x) for x in request.POST.getlist('listOfGrossMass') if x],
                listOfLiquidMass='[20,10]',#[float(x) for x in request.POST.getlist('listOfLiquidMass') if x],
                listOfReturned='[true, false]',#[x == 'on' for x in request.POST.getlist('listOfReturned')],
                listOfCounterProof='[true, true]',#[x == 'on' for x in request.POST.getlist('listOfCounterProof')],
                listOfEntranceSeal='[123456, 45678]',#request.POST.getlist('listOfEntranceSeal'),
                listOfExitseal='[456987, 987654]',#request.POST.getlist('listOfExitseal'),
                listOfpackagingAndMorphology='[m1, m2]',#request.POST.getlist('listOfpackagingAndMorphology'),
                listOfResultOfExams='[cocaina, thc]',#request.POST.getlist('listOfResultOfExams'),
            )
