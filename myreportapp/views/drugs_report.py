    ...
    if request.method == 'POST':
        ...
        try:
            new_report = DrugReport(
                ...
                protocol_number=request.POST.get('protocol'),
                designated_date=request.POST.get('releaseDate'),
                exam_objective='Constatação Provisória',
                occurrence_nature='Entorpecente',
                police_station=request.POST.get('policestation'),
                requesting_authority=request.POST.get('authority'),
                activation_date=request.POST.get('serviceDate'),
                activation_time=request.POST.get('serviceHour'),
                service_date=request.POST.get('releaseDate'),
                service_time=request.POST.get('releaseHour'),
                director=user_data.director,
                nucleus=user_data.unit,
                team=user_data.team,
                reporting_expert=user_data.full_name,
                assistant_expert='Não se aplica',
                photographer='Não se aplica',
                draftsman='Não se aplica',
                allSubstances=request.POST.getlist('allSubstances'),
                materialReceivedObservations=request.POST.getlist('materialReceivedObservations'),
                listOfEnvolvedPeople=request.POST.getlist('listOfEnvolvedPeople'),
                listItensLabels=request.POST.getlist('listItensLabels'),
                listOfPackagings=request.POST.getlist('listOfPackagings'),
                listOfMorphology=request.POST.getlist('listOfMorphology'),
                listOfGrossMass=[float(x) for x in request.POST.getlist('listOfGrossMass') if x],
                listOfLiquidMass=[float(x) for x in request.POST.getlist('listOfLiquidMass') if x],
                listOfReturned=[x == 'on' for x in request.POST.getlist('listOfReturned')],
                listOfCounterProof=[x == 'on' for x in request.POST.getlist('listOfCounterProof')],
                listOfEntranceSeal=request.POST.getlist('listOfEntranceSeal'),
                listOfExitseal=request.POST.getlist('listOfExitseal'),
                listOfpackagingAndMorphology=request.POST.getlist('listOfpackagingAndMorphology'),
                listOfResultOfExams=request.POST.getlist('listOfResultOfExams'),
            )
